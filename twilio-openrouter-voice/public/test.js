...
async function apiCall(endpoint, options = {}) {
    try {
        ...
        if (isSuccess) {
            ...
            log(`❌ ${endpoint} - ${data.error || data.status || 'Unknown error'}`, 'error');
        }
...
async function authenticateCalendar() {
    ...
    if (result.success && result.authUrl) {
        ...
        log('📋 After authentication, copy the code and paste it in the input field', 'info');
    }
    showStatus('calendar-status', JSON.stringify(result, null, 2), result.success);
}
...
async function authenticateGmail() {
    try {
        ...
        if (response.authUrl) {
            ...
            showStatus('gmail-status', '✅ Gmail auth URL generated - opening in new tab', true);
            window.open(response.authUrl.authUrl, '_blank');
        } else {
